...
cookie compliance. The backend layer that once lived in this repo
(Postgres/Drizzle, auth, waitlist/contact/newsletter APIs, Resend email,
Stripe, i18n dictionaries) was **removed from master** — the site is a pure
static export on GitHub Pages, the build is a plain `next build`, and
deploys are green. That backend surface would return only as part of a
deliberate hosting move to Vercel (Phase 0 decision below).

What it is missing versus a $100k site, bluntly:

1. **No commissioned imagery.** The phone mockup is CSS and the map is a
   fake SVG. Interim vehicle/lifestyle photos now exist in `public/images/`,
   but they are stopgap shots — not the branded 3D renders and professional
   Costa del Sol photography of `docs/ASSET-BRIEF.md`.
...
4. **Product features are decorative.** The waitlist button doesn't submit
   anywhere; there is no backend on master to power it.
5. **No quality gates.** No visual regression, no Lighthouse budget, no axe
   CI; regressions are caught by eyeballs or not at all.

## Phase 0 — Keep deploys green (prerequisite, ~0 new cost)

The original Phase 0 (PR #1's deploy-pipeline and middleware fixes) is
**superseded**: master healed independently — the backend/API layer and
middleware were deleted, the build wrapper is gone, and GitHub Pages deploys
are green. What remains of Phase 0:

- **Keep deploys green.** Every PR must pass both build flavors (server
  `npm run build` and `NEXT_OUTPUT=export CUSTOM_DOMAIN=true npm run build`)
  via `.github/workflows/ci.yml`; master pushes deploy via
  `.github/workflows/deploy.yml`.
- **Decide the hosting move to Vercel** when (and only when) a workstream
  actually needs a server: live waitlist/contact APIs, security headers,
  image optimization, preview deploys. The backend layer would be
  reintroduced as part of that move — it no longer exists on master.
...
- **Working waitlist/newsletter:** wire DownloadCTA + footer to a
  `/api/waitlist` endpoint + Resend confirmation emails (requires the
  Vercel move — the API layer does not exist on master). Double-opt-in,
  GDPR-clean, admin export.
...
- **Per-city landing pages** (`/marbella`, `/estepona`, …) generated from
  ...
  search strategy.
- **Careers + press kit pages** with real content; contact form on a
  `/api/contact` endpoint (also Vercel-gated).
...
| Week | Workstream |
|------|-----------|
| 0 | Keep deploys green; decide Vercel move timing |
